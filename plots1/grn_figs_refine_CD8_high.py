...
#df.to_csv((out_dir_agg + "cd8_high_tf_annotation.csv"), index=False)  # https://manual.cytoscape.org/en/3.4.0/Node_and_Edge_Column_Data.html
...
links1_update = cut_by_scores(links=links1, scores_df=li_links_scores, score_threshold=score_threshold, edge_weights=True, as_df=True)
links2_update = cut_by_scores(links=links2, scores_df=yost_links_scores, score_threshold=score_threshold, edge_weights=True, as_df=True)
links3_update = cut_by_scores(links=links3, scores_df=wang_links_scores, score_threshold=score_threshold, edge_weights=True, as_df=True)

links_all = pd.concat([links1_update, links2_update, links3_update], ignore_index=True)
links_all['importance'] = [float(i.iloc[0]) for i in list(links_all['importance'])]
grouped = links_all.groupby(['TF', 'target'])['importance'].mean().reset_index()
summary_rows = grouped.copy()
df_summary = pd.concat([links_all[~(links_all['TF'].isin(summary_rows['TF']) & links_all['target'].isin(summary_rows['target']))], summary_rows], ignore_index=True)  # this line may be unnecessary
df_summary.columns = ['source', 'target', 'weight']
G = nx.from_pandas_edgelist(df_summary, 'source', 'target', ['weight'])
nx.write_graphml(G=G, path=(out_dir_agg + "cd8_high_agg_cutoff25_edgeweights.graphml"))
