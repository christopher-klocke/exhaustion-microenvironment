...
### remove PBMC samples
sample_id_list = [x for x in set(list(adata.obs[sample_name])) if x[-4:] != 'PBMC'] ## need to adjust this for wang dataset ########
...
"""
sns.distplot(all_points, bins=100)

plt.axvline(x=threshold, color='red') ## plot threshold line 

plt.savefig('li_cd8_dens_v1.pdf')
#plt.show(block=True)
...
generate joint histogram / density plot -- INSET

"""

sns.distplot(all_points, bins=300)

plt.axvline(x=threshold, color='red') ## plot threshold line 

plt.xlim([0, 0.2])

plt.savefig('li_cd8_dens_inset_v1.pdf')
#plt.show(block=True)
